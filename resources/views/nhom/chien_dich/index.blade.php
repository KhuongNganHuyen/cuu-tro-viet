...
@if ($dangLoc)
  <div class="alert alert-info d-flex justify-content-between align-items-center gap-3">
    ...
    <a href="{{ url('/nhom/' . $nhom->idNhom . '/chien-dich' . (request('tuKhoa') ? '?tuKhoa=' . urlencode(request('tuKhoa')) : '')) }}"
       class="btn btn-sm btn-light flex-shrink-0">
      Xóa bộ lọc
    </a>
  </div>
...
<div class="card">
  ...
  <div class="card-body">
    <form id="boLocChienDichForm"
          action="{{ url('/nhom/' . $nhom->idNhom . '/chien-dich') }}"
          method="GET"
          autocomplete="off">
            ...
            <th class="filter-heading-cell" style="width: 140px;">
              <div class="dropdown">
                ...
                <ul class="dropdown-menu filter-dropdown-menu">
                  ...
                  @foreach (['Sắp diễn ra', 'Đang diễn ra', 'Đang hoạt động', 'Tạm ngưng', 'Hoàn thành'] as $trangThaiLoc)
                    <li>
                      <button type="button"
              ...
              <td class="text-center">
                @php
                  $classTrangThai = match ($chienDich->trangThai) {
                      'Đang hoạt động', 'Đang diễn ra' => 'status-active',
