...
<div class="card">
  ...
  <div class="card-body">
    ...
    <form action="{{ url('/admin/nguoi-dung') }}" method="POST">
      @csrf
      ...
      <div class="row">
        <div class="col-md-6 mb-3">
          ...
          <select name="vaiTro" class="form-control">
            <option value="Người dùng" {{ old('vaiTro') == 'Người dùng' ? 'selected' : '' }}>Người dùng</option>
      ...
      <div class="mb-3">
        ...
        <input type="text" name="anhDaiDien" class="form-control" value="{{ old('anhDaiDien') }}" placeholder="Tạm thời để trống">
        <small class="text-muted">Phần upload ảnh sẽ làm sau.</small>
      </div>

      <div class="d-flex gap-2">
        <button type="submit" class="btn btn-primary">Lưu</button>
        <a href="{{ url('/admin/nguoi-dung') }}" class="btn btn-secondary">Quay lại danh sách</a>
      </div>
    </form>
  </div>
</div>
